...
#define GROWTHFACTOR 2
	...
	struct vec {
		...
		uint4 size;      /* the capacity of vector */
		uint4 pos;       /* where we are in vector */
		uint4 len;       /* the current length of vector */
	...
	int vec_init_inplace(uint4 initsize, struct vec *vec);
