...
int alignments_expandCluster(struct alignment* alignment, struct PSSMatrix PSSMatrix);
                              ...
                              struct ungappedExtension* ungappedExtension, struct PSSMatrix PSSMatrix)
{
	...
	while (currentExtension != NULL) {
		...
		if (currentExtension->status == ungappedExtension_DELETED &&
			...
			// Find the seed
			ungappedExtension_findSeed(restoreExtension, PSSMatrix, alignment->subject);
		} else {
...
void alignments_addGoodAlignment(int4 highestNominalScore, struct alignment* alignment)
{
	...
	goodAlignment = memSingleBlock_newEntry(alignments_goodAlignments);
...
int alignments_expandCluster(struct alignment* alignment, struct PSSMatrix PSSMatrix)
{
	...
	int4 childNum, numNewAlignments = 0;
	uint4 numChildren = 0;
